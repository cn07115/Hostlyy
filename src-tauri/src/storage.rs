...
pub fn toggle_profile_active(app: AppHandle, id: String) -> Result<(), String> {
    crate::log_tray_str(&format!("[toggle_profile_active] enter, id={}", id));
    toggle_profile_active_internal(&Context::Tauri(&app), &id)?;
    crate::log_tray_str("[toggle_profile_active] internal toggle done, calling apply_config");
    apply_config(app.clone())?;
    crate::log_tray_str("[toggle_profile_active] apply_config Ok, rebuild_tray_menu + schedule_sync");
...
pub fn apply_config_internal(ctx: &Context) -> Result<(), String> {
    ...
    let active_count = config.profiles.iter().filter(|p| p.active).count();
    let total = config.profiles.len();
    crate::log_tray_str(&format!("[apply_config_internal] start, total={} active={}", total, active_count));
    ...
    for profile in &config.profiles {
        if profile.active {
            let body = read_profile(&profile.id);
            crate::log_tray_str(&format!("[apply_config_internal] active profile: id={} name={} body_len={}", profile.id, profile.name, body.len()));
    ...
    crate::log_tray_str(&format!("[apply_config_internal] calling save_system_hosts, merged_len={}", merged_content.len()));
    let r = crate::hosts::save_system_hosts(merged_content.clone());
    crate::log_tray_str(&format!("[apply_config_internal] save_system_hosts returned: {:?}", r.as_ref().map(|_| "Ok")));
    r
}
