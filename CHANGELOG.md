...
### 修复
- 修复"开机自启"实际上没起作用的问题：之前注册表写对了，但 app 启动后被 CLI 参数解析当成未知命令错误退出
- 修复开机自启时主窗口会弹出来的问题：自启时应该只在托盘常驻，不弹主窗口

### 变更
- 加单实例锁：再次双击启动时自动聚焦已运行的实例，不再多开
- bump 1.3.9 → 1.3.10，方便用户从 v1.3.9 直接 auto-upgrade
...
- 修复「检查更新」走代理无法拉取新版本
- 修复已是最新版本却提示更新的问题
- 修复「检查更新」期间主程序卡死（后台异步执行，不再阻塞 UI）
- 修复托盘菜单不显示哪些 host 已启用
- 修复托盘点击 host 时主窗口弹出（改为系统通知，不再弹窗）
- 修复托盘切换 host 时系统通知不弹的问题（启动时显式请求通知权限，并输出错误日志）
- 托盘切换通知文案区分「已启用 / 已禁用」，并暴露错误日志（便于排查 hosts 是否真写入）
- 托盘和通知相关日志同时写入 `%APPDATA%\com.hostly.switcher\hostlyy-tray.log`（Windows GUI app stderr 看不到，文件 log 才能查）
- 优化托盘菜单选中项的显示效果
- 升级时自动迁移老版本的代理地址
